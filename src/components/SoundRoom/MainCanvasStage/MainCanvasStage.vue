 <template>
  <div
    ...
    class="canvas-grid relative border border-[var(--sr-panel-border)] dark:border-[var(--sr-panel-border)] dark:border-2 flex items-center justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 shadow-[var(--sr-shadow)] dark:shadow-none"
...
.canvas-grid {
  ...
  background-image:
    ...
    linear-gradient(to bottom, var(--sr-grid) 1px, transparent 1px); /* Adjust line opacity */
}
...
@media (prefers-color-scheme: dark) {
  .canvas-grid {
    ...
    background-image:
      linear-gradient(to right, var(--sr-grid) 2px, transparent 2px),
      linear-gradient(to bottom, var(--sr-grid) 2px, transparent 2px); /* Adjust dark mode opacity separately */
  }
}
</style>
